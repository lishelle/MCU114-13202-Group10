...
class CardMeaningsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val copyForGeminiButton = view.findViewById<Button>(R.id.btn_copy_for_gemini)
        ...
        if (cardsJson != null) {
            ...
            copyForGeminiButton.setOnClickListener {
                // Combine only the card name and orientation for the query
                val textForQuery = (0 until cards.length()).joinToString("\n") { i ->
                    ...
                    val title = if (cards.length() > 1) "${listOf("過去", "現在", "未來")[i]}： " else ""
                    
                    "$title$name $orientation"
                }
                
                val fullPrompt = "請用塔羅牌的牌義，為我解讀以下抽牌結果：\n\n$textForQuery"

                // 1. Copy the prompt to the clipboard
                val clipboard = requireActivity().getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
                val clip = ClipData.newPlainText("Tarot Reading for Gemini", fullPrompt)
                clipboard.setPrimaryClip(clip)
                Toast.makeText(requireContext(), "已複製解讀請求至剪貼簿", Toast.LENGTH_SHORT).show()

                // 2. Construct the Gemini URL and open it directly
                val geminiUrl = "https://gemini.google.com/?q=" + Uri.encode(fullPrompt)
        ...
        backToHomeButton.setOnClickListener {
            // Clear the entire back stack and go to Home
    ...
    private fun addMeaningViews(inflater: LayoutInflater, container: LinearLayout, cards: JSONArray) {
        ...
        for (i in 0 until cards.length()) {
            ...
            val isReversed = card.optBoolean("reversed", false)
            val orientation = if (isReversed) "(逆位)" else "(正位)"
            val meaning = if (isReversed) card.optString("meaning_rev", "牌義不詳") else card.optString("meaning_up", "牌義不詳")
            
            val cardMeaning = if (cards.length() > 1) {
